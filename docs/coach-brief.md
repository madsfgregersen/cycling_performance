...
- The **dashboard Now page shows the morning verdict explanation** (story 1) as a coach block in the readiness hero — `headline` + `why` only (the evaluation of the data, without the actionable `note`). To keep page loads free of live LLM cost, this is served from a **cache, not a fresh call**: when `telegram.send_morning_verdict` generates the explanation it also writes it as an `IntegrationLog` row (`source="coach"`, `event="morning_brief"`, summary = JSON of date/headline/why/note — no new table). `dashboard.get_dashboard_data` returns the latest such row under `morning_brief`; the Now page hides the block when none is cached yet. The `/coach/verdict-preview` endpoint still generates on demand and is unchanged.
- **Recorded rides on the Plan calendar + cached ride debrief.** The Plan week grid now renders recorded rides (Bucket 1) as tiles alongside planned workouts (`GET /rides?days=`), merging planned + actual on the same date into one tile where the **actual TSS is the number that counts** (the merge is display-only — the two buckets stay separate rows, per principle 2). The post-ride debrief is **cached like the morning brief**: `coach_ride.cache_ride_brief` computes `explain_ride` once and stores it as an `IntegrationLog` row (`source="coach"`, `event="ride_brief"`, summary = `strava_activity_id=<id> | {headline,why,note}` JSON — the id prefix + `" |"` delimiter makes the per-ride lookup collision-safe). It's called on ride-landed (`strava_webhook`) so the tile has it even when the Telegram debrief is off, and `send_post_ride_debrief` reuses the same cache (no second LLM call). The tile's read-only detail (`GET /rides/{strava_activity_id}`) shows the cached debrief **plus the athlete's RPE/feel reply** for that ride (from `telegram_checkins.ride_id`, story 4) — objective + subjective together. The cached debrief is written at ride-land time, so it reflects the data, not the later RPE reply, which is shown alongside rather than folded into the coach's prose.
