...
def explain_ride(db: Session, ride) -> dict:
    ...
    prompt = (
        ...
        + "\n\nWrite a short post-ride debrief. Interpret efficiency factor "
        "(power per heartbeat -- higher is more efficient). For decoupling: "
        "decoupling_pct is aerobic decoupling, and the SIGN matters. POSITIVE "
        "means heart rate drifted UP relative to power through the ride -- a "
        "fade, i.e. LESS durable (fatigue, heat, under-fuelling, or riding "
        "beyond your aerobic base). NEGATIVE or near zero means efficiency held "
        "or improved -- you stayed fresh. Rough scale: under ~5% (or negative) "
        "is well-coupled and good; ~5-10% is moderate drift; over ~10% is a "
        "real fade. Judge it against the ride type -- low decoupling matters "
        "most on a long steady endurance ride (durability); a hard interval day "
        "naturally shows more, so don't over-alarm, but NEVER call a positive "
        "figure 'stable' or 'good'. decoupling_pct is null if there wasn't "
        "enough data to call it -- don't mention it in that case. If a planned "
