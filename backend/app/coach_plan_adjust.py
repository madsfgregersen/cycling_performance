...
INTENT_SCHEMA = {
    ...
    "properties": {
        "intent": {
            "type": "string",
            "enum": ["disruption", "plan_structure", "compile", "question", "generic"],
            "description": (
                ...
                "'plan_structure' if they want to reshape the plan's DESCRIPTION itself -- a week's or "
                "phase's focus, label, volume, or intensity, whether they name a specific week/phase "
                "('make week 3 a recovery week', 'make the base block harder') or speak generally about "
                "the whole plan ('make my plan easier', 'can we add more rest overall', 'I want to push "
                "harder'). This changes what a week is ABOUT, not the concrete sessions on the calendar. "
                "When in doubt between disruption and plan_structure, prefer whichever fits better -- "
                "both are safe, reasoned choices here. "
                "'compile' if they want actual, concrete workouts PUT ON the calendar -- for a single "
                "day ('what should I do today?', 'give me a session for Thursday', 'plan tomorrow's "
                "ride') or a whole week/block ('fill in the base block', 'compile week 3', 'schedule "
                "this week', 'do the next week', 'continue filling the plan'). This turns the plan's "
                "intent into real scheduled sessions. Note the contrast with plan_structure: 'fill in "
                "the base block' is compile (create sessions), but 'make the base block easier' is "
                "plan_structure (edit its description). Compile is also not adjusting an existing "
                "session (that's disruption) or merely asking about data (that's question). "
                "'question' if they're asking about their own data or progress rather than asking for "
                "anything to change or to be scheduled -- HRV/sleep/resting-HR trends, recent rides, "
                "how a metric is moving, whether they're on track for the goal, why a verdict looked a "
                "certain way, how many hours/how much TSS this week, etc. "
